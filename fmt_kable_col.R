...
fmt_kable_col <- function(df, col, sigfig, maxdigits, maxzeros) {
  ...
  for (iCol in col) {
    ...
    for (iRow in 1:nRows) {
      ...
      if (grepl('e',newRowVal,ignore.case = TRUE)) {
        newRowVal <-  sub('E\\+0','\\\\times 10^{',newRowVal,ignore.case = TRUE)
        newRowVal <-  sub('E-0','\\\\times 10^{-',newRowVal,ignore.case = TRUE)
        newRowVal <-  sub('E\\+','\\\\times 10^{',newRowVal,ignore.case = TRUE)
        newRowVal <-  sub('E-','\\\\times 10^{-',newRowVal,ignore.case = TRUE)
